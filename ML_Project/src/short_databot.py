...
from src.utils.mining_data_tb import final_add_short_features
from src.utils.models import final_preprocessing
model_path = path + os.sep + 'src' + os.sep + 'models' + os.sep + 'short_CNN.h5'
    ...
    if counter == 0:
        ...
        if len(_zmq._Market_Data_DB['EURUSD_M1']) > len(date):
            if len(predictions)> 0:
                ...
                close_prices.append(_zmq._Market_Data_DB['EURUSD_M1'][list(_zmq._Market_Data_DB['EURUSD_M1'].keys())[-1]][4])
                counter += 1
                print(f'{len(date)}/30 of data collected to start predicting')

            if len(date) >= 30 and len(compared_prices) == 0:
            ...
            if len(date) >= 30 and len(compared_prices) > 0:
                ...
                pred_df = final_add_short_features(pred_df).iloc[-10:, :]
...
historic_path = path + os.sep + 'data' + os.sep + 'historico_corto1.csv'
